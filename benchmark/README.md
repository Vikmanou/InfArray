...
Micro-benchmarks comparing **InfArray** against a **Luau table** on the
...
- Each `cases/*.bench.luau` file is a **Spec**: one workload (`n` elements) and a
  set of competing `variants` that each perform it. See `cases/pushback.bench.luau`
  for the simplest example.
- For every variant the harness runs `setup` (untimed) then the timed `run`,
  discards a couple of warmup samples, keeps several timed samples, and reports
  the **best** (least-noisy) one. Timing uses Lute's high-resolution
  `@lute/time`.
- The `Luau table` variant is the **baseline**. Every other variant is reported
  as a multiple of it.
